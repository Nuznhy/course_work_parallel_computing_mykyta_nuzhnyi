...
# class OpenAddressingMultiShared(Namespace):
#     _exposed_ = ('__getattribute__', '__getattr__', '__setattr__', '__init__', 'getitem', 'insert', 'find', '__getitem__', '__setitem__')
#
#     def getitem(self, key, default=None):
#         callmethod = object.__getattribute__(self, '_callmethod')
#         return callmethod('getitem', (key, default))
#
#     def insert(self, key, value, hash_key=None):
#         callmethod = object.__getattribute__(self, '_callmethod')
#         return callmethod('insert', (key, value, hash_key))
#
#     def find(self, key):
#         callmethod = object.__getattribute__(self, '_callmethod')
#         return callmethod('find', (key))


class OpenAddressingMulti:
...
class HashSet:
    def __init__(self):
        self.hash_set_table = OpenAddressingMulti(10)
        self.present = object()

    def add(self, value):
        if self.__contains__(value):
            self.hash_set_table[value] = self.present
            return 0
        else:
            self.hash_set_table[value] = self.present
            return 1
